...
**R**esting-state and task-based **P**erceptual and **O**ther **E**xperience (**RPOE**) — 7 Tesla MRI Processing and Analysis

This repository contains the full pipeline for processing, analyzing, and visualizing 7T MRI data from the RPOE study, including structural (T1w/T2w) and functional (resting-state and task-based) MRI modalities. Analyses span anatomical volumetrics, functional signal characterization, task-fMRI design and activation modeling, functional connectivity, network controllability, and mitochondrial mapping — all linked to behavioral and cognitive phenotypes (language, IQ).
...
| `01_02_DICOM-conversion.sh` | Convert DICOMs to NIfTI format (BIDS-compatible) |
...
| `04_04_NCT-run.R` | Run network controllability theory (NCT) analysis on functional connectivity matrices |
| `04_04_NCT.py` | Python helper for NCT computations (average/modal controllability) |
| `04_05_mito-maps.R` | Generate and analyze mitochondrial function proxy maps (T2/T1-derived) |
...
- [FSL](https://fsl.fmrib.ox.ac.uk/fsl/) — skull stripping, registration, preprocessing
- [AFNI](https://afni.nimh.nih.gov/) — task-fMRI GLM modeling and group analysis
- [SPM12](https://www.fil.ion.ucl.ac.uk/spm/) (MATLAB) — second-level voxelwise analyses
- [dcm2niix](https://github.com/rordenlab/dcm2niix) — DICOM to NIfTI conversion

### Programming Languages & Packages

**R**
- `tidyverse`, `ggplot2` — data wrangling and visualization
- `oro.nifti`, `RNifti` — NIfTI I/O
...
- `networkx` — network controllability computations

**MATLAB / SPM**
- SPM12 batch scripting for second-level GLMs

### HPC
- SLURM-based job scheduling (see `99_pipelines-submission.sh`)
